...
Feature: Verification of available payment types in cart page
  ...
  Scenario: User sets store as Florida store, search for an item, add item to cart and navigate to Cart Page
    ...
    Then user set the closet store by "33544"
    Then User search for a SKU "K600002651" and clicks on it
    Then User adds items to cart
    Then User navigate to cart page
    Then User clicks on Secure Check out button
    Then User enters customer info "FName" , "LName" ,"100 Main St", "Apt 12", "Wesley Chapel", "Florida", "33544", "6126543265", "qa@test.com" on Secure Checkout Delivery Page
